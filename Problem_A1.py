...
#---Define the required library/packages
import numpy as np
import tensorflow as tf



#---Make Callbacks Class to stop within the desired loss < 1e-4
class myCallback(tf.keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs={}):
        if logs.get('loss')< 1e-4:
            print("\nAlready reached the desired loss bellow 1e-4")
...
def solution_A1():
    ...
    # ---compile the model and fit arrayX and arrayY to it
    # ---Make the Deep Learning Configuration
    model = tf.keras.models.Sequential([
        tf.keras.layers.Dense(units=1, input_shape=[1]),
        tf.keras.layers.Dense(units=1, activation='linear')  # ---input and output layer at once
    ])

    # --Train it in 100 epochs use callbacks
